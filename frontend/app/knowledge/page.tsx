...
function ModeSwitchPill({
  ...
  return (
    <button
      ...
      className={`pointer-events-auto inline-flex shrink-0 items-center gap-1 rounded-full border px-3 py-1.5 text-sm font-medium shadow-sm transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-400/60 disabled:cursor-not-allowed disabled:opacity-45 ${
        deep
          ? 'border-teal-200 bg-teal-50 text-teal-800 hover:bg-teal-100'
          : 'border-gray-200 bg-gray-50 text-gray-700 hover:bg-gray-100'
      }`}
    >
      <svg width="15" height="15" viewBox="0 0 16 16" fill="none" className="shrink-0 opacity-90" aria-hidden>
...
function SourcesPanel({ hits }: { hits: VectorHit[] }) {
  ...
  return (
    <div className="mt-3">
      <button type="button" onClick={() => setOpen(!open)} className="flex items-center gap-1.5 rounded-md border border-gray-200 bg-white px-2.5 py-1.5 text-sm font-medium text-gray-500 transition-colors hover:border-teal-300 hover:text-teal-600">
          ...
          {hits.map((h, i) => (
            <div key={h.id || i} className="rounded-lg border border-gray-100 bg-gray-50 px-3 py-2.5 transition-colors hover:border-teal-200">
              <div className="flex items-center justify-between gap-2">
                <span className="flex min-w-0 flex-1 flex-wrap items-center gap-1.5 text-sm font-medium text-gray-500">
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded bg-teal-50 text-xs font-bold text-teal-600">{i + 1}</span>
                  {h.book_name ? <span className="truncate text-gray-600">《{h.book_name}》</span> : null}
                  <span>第 {h.chapter_num} 章</span>
                </span>
                <span className="rounded-full bg-teal-50 px-2 py-0.5 font-mono text-xs text-teal-600">{h.score.toFixed(4)}</span>
              </div>
              <p className="mt-1.5 text-sm leading-relaxed text-gray-600">{h.contentPreview}</p>
            </div>
          ))}
        </div>
      )}
...
function TypingIndicator() {
  return (
    <div className="flex items-center gap-1 py-2">
      ...
      <span className="ml-2 text-sm text-gray-400">正在检索知识库并生成回答...</span>
    </div>
  );
}
...
export default function KnowledgePage() {
  ...
  return (
    <div className="flex h-screen bg-gray-50 text-base text-gray-800">
      {/* Sidebar */}
      <aside className={`flex flex-col border-r border-gray-200 bg-teal-600 text-white transition-all duration-300 ${sidebarOpen ? 'w-60' : 'w-0 overflow-hidden'}`}>
        ...
        <div className="px-3 pb-3">
          <button type="button" onClick={() => { setActiveId(null); setError(null); }} className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-white/15 py-2.5 text-base font-medium transition-colors hover:bg-white/25">
        ...
        <div className="flex-1 overflow-y-auto px-2 text-base">
          {groups.map((g) => (
            <div key={g.label} className="mb-2">
              <p className="px-1 py-1 text-xs font-medium uppercase tracking-wider text-white/50">{g.label}</p>
              {g.items.map((c) => (
                <div key={c.id} className={`group flex cursor-pointer items-center justify-between rounded-md px-2 py-2 transition-colors ${activeId === c.id ? 'bg-white/20' : 'hover:bg-white/10'}`} onClick={() => { setActiveId(c.id); setError(null); }}>
        ...
        <div className="mt-auto shrink-0 border-t border-white/10 px-2 py-3">
          <button
            ...
            className="flex w-full items-center justify-center gap-1.5 rounded-lg py-2.5 text-sm font-medium text-white/95 transition-colors hover:bg-white/10 lg:hidden"
          >
          ...
          <button
            ...
            className="hidden w-full items-center justify-center gap-1.5 rounded-lg py-2.5 text-sm font-medium text-white/95 transition-colors hover:bg-white/10 lg:flex"
          >
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden>
              <path d="M3 2.5h7a2 2 0 012 2v7a2 2 0 01-2 2H3a1 1 0 01-1-1v-9a1 1 0 011-1z" stroke="currentColor" strokeWidth="1.2" />
              <path d="M5.5 5h4M5.5 8h4M5.5 11h2.5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
        ...
        {!sidebarOpen && (
          <div className="flex items-center gap-2 border-b border-gray-200 px-4 py-2">
            ...
            <button
              type="button"
              onClick={() => setKbDrawerOpen(true)}
              className="rounded-md px-2 py-1.5 text-sm font-medium text-teal-700 hover:bg-teal-50 lg:hidden"
            >
              知识库
            </button>
          </div>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto">
          {messages.length === 0 && !loading ? (
            <div className="flex h-full flex-col items-center justify-center px-4">
              ...
              <p className="mt-2 text-base text-gray-500">基于武侠小说知识库，输入问题即可检索相关内容</p>

              <div className="mt-6 flex overflow-hidden rounded-full border border-gray-200 bg-white text-base">
                <button type="button" onClick={() => setMode('quick')} className={`px-6 py-2 font-medium transition-colors ${mode === 'quick' ? 'bg-teal-600 text-white' : 'text-gray-500 hover:text-gray-700'}`}>快速问答</button>
                <button type="button" onClick={() => setMode('deep')} className={`px-6 py-2 font-medium transition-colors ${mode === 'deep' ? 'bg-teal-600 text-white' : 'text-gray-500 hover:text-gray-700'}`}>深度分析</button>
              </div>
              <p className="mt-3 max-w-xl text-center text-base leading-relaxed text-gray-600">{MODE_DESCR[mode]}</p>

              <div className="mt-6 w-full max-w-xl">
                <div className="relative rounded-xl border border-gray-200 bg-white shadow-sm">
                  <textarea ref={inputRef} value={input} onChange={(e) => { setInput(e.target.value); autoResize(e.target); }} onKeyDown={handleKeyDown} placeholder="输入你的问题，例如：乔峰的降龙十八掌有多厉害？" rows={3} className="w-full resize-none rounded-xl bg-transparent px-4 pt-3 pb-11 text-base text-gray-700 outline-none placeholder:text-gray-400" />
                  <div className="pointer-events-none absolute bottom-2 left-3 right-3 flex items-center justify-between gap-2">
                    ...
                    <button
                      ...
                      className="pointer-events-auto rounded-lg bg-teal-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-teal-500 disabled:opacity-40"
                    >
                      提问
                    </button>
                  </div>
              ...
              <div className="mt-4 flex flex-wrap justify-center gap-2">
                {SUGGESTIONS.map((s) => (
                  <button key={s} type="button" onClick={() => void handleSubmit(s)} className="rounded-full border border-gray-200 bg-white px-4 py-1.5 text-sm text-gray-500 transition-colors hover:border-teal-300 hover:text-teal-600">{s}</button>
                ))}
              </div>
            </div>
          ) : (
            <div className="mx-auto max-w-3xl px-4 py-6">
                ...
                <div key={`${msg.ts}-${i}`} className="mb-5">
                  {msg.role === 'user' ? (
                    <div className="flex justify-end">
                      <div className="max-w-[80%] rounded-2xl rounded-br-sm bg-teal-600 px-4 py-3 text-base leading-relaxed text-white shadow-sm">{msg.content}</div>
                    </div>
                  ) : (
                    <div className="flex gap-3">
                      <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-teal-50 text-teal-600">
                        <svg width="15" height="15" viewBox="0 0 16 16" fill="none"><path d="M8 1L9.5 6.5L15 8L9.5 9.5L8 15L6.5 9.5L1 8L6.5 6.5L8 1Z" fill="currentColor" /></svg>
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="prose prose-base max-w-none text-gray-700 prose-headings:text-gray-800 prose-p:leading-relaxed prose-strong:text-gray-900 prose-code:rounded prose-code:bg-gray-100 prose-code:px-1 prose-code:py-0.5 prose-code:text-teal-700">
                          <ReactMarkdown remarkPlugins={[remarkGfm]}>{msg.content}</ReactMarkdown>
                        </div>
                        {msg.agentTrace?.mode === 'deep' && msg.agentTrace.subQueries && msg.agentTrace.subQueries.length > 0 && (
                          <div className="mt-3 rounded-lg border border-teal-100 bg-teal-50/60 px-3 py-2.5 text-sm text-teal-900">
                            ...
                            <ul className="mt-1.5 list-disc space-y-0.5 pl-4 text-sm leading-snug text-teal-800/90">
              ...
              {error && (
                <div className="mb-5 ml-10 rounded-lg border border-red-200 bg-red-50 px-4 py-2.5 text-base text-red-600">{error}</div>
              )}
            </div>
          )}
        </div>
        ...
        {messages.length > 0 && (
          <div className="border-t border-gray-200 bg-white px-4 py-3">
            <div className="mx-auto flex max-w-3xl items-end gap-2">
              <div className="relative flex-1">
                <textarea
                  ...
                  placeholder="继续提问..."
                  rows={1}
                  className="w-full resize-none rounded-xl border border-gray-200 bg-gray-50 px-3 py-3 pb-11 pr-12 text-base text-gray-700 outline-none transition-colors placeholder:text-gray-400 focus:border-teal-400 focus:bg-white"
                />
                <div className="pointer-events-none absolute inset-x-0 bottom-0 flex items-center justify-between gap-2 px-2 pb-2">
                  ...
                  <button
                    ...
                    onClick={() => void handleSubmit()}
                    disabled={loading || !input.trim()}
                    className="pointer-events-auto flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-teal-600 text-white transition-colors hover:bg-teal-500 disabled:bg-gray-300 disabled:text-gray-400"
                  >
            ...
            <div className="mx-auto mt-2 max-w-3xl space-y-1.5 px-1">
              <p className="text-center text-sm leading-relaxed text-gray-600">{MODE_DESCR[mode]}</p>
              <p className="text-center text-xs text-gray-400">Enter 发送 · Shift+Enter 换行 · 当前：{mode === 'quick' ? '快速问答' : '深度分析'}</p>
            </div>
          </div>
        )}
        </div>
        ...
        {kbRailOpen ? (
          <aside className="hidden w-[min(100%,400px)] shrink-0 flex-col border-l border-gray-200 bg-white lg:flex">
            <div className="flex shrink-0 items-center justify-between gap-2 border-b border-gray-100 px-3 py-2.5">
              <span className="text-base font-semibold text-gray-800">我的知识库</span>
              <button
                type="button"
          ...
          >
            ...
            <span className="text-xs font-semibold leading-tight [writing-mode:vertical-rl]">知识库</span>
          </button>
        )}
      </div>
